...
| 인구통계        | InsightFace (buffalo_l, age + gender) | 0.7+   |
| 인구통계 런타임 | ONNX Runtime                          | 1.16+  |
...
| **`roi_x_min`**, **`roi_y_min`**, **`roi_x_max`**, **`roi_y_max`** | int  | ❌   | 관심구역(ROI) 절대 픽셀 좌표. 생략 시 default 비율 사용 |
...
| ROI Dwell             | 클라이언트가 전달한 절대 좌표(**`roi_abs`**) 우선, 없으면 default 비율로 직사각형 정의. 추적 ID가 ROI 내 머문 시간 측정 |
| 인구통계 ID 캐싱      | 각 추적 ID당 최대 5회만 얼굴 추론 → 평균/최빈값으로 고정. 얼굴 미검출 ID는**`unknown`**                                      |
...
- **얼굴 모델 한계 정직 표기**: 하이앵글 CCTV에서 얼굴이 안 잡히면 **`unknown`**
...
- [X] ROI 동적 입력 (클라이언트가 영상마다 관심구역 좌표 전달)
- [X] 인구통계 ID 캐싱 (InsightFace) — 얼굴 검출 시 age/gender, 미검출 시 unknown
